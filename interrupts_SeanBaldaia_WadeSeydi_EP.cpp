...
//reorganize ready processes based on EP (lowest PID first)
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) {
        ...
        for (auto counter = 0; counter < wait_queue.size(); counter++){
            ...
            if (flag){
                ...
                ready_queue.push_back(current); //Add the process to the ready queue
                counter--;
        ...
        if (running.state != RUNNING){
            EP(ready_queue);                                            //reorganize ready queue
...
int main(int argc, char** argv) {
    ...
    if(argc != 2) {
        ...
        std::cout << "To run the program, do: ./interrupts inputs.txt" << std::endl;
        return -1;
    }
